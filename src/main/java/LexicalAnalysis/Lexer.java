...
public class Lexer {
  ...
  public Token getNextToken() {
    ...
    return getEofToken()
        ...
        .orElseThrow(
            () -> new TokenGenerationException(code.substring(startPosition, currentPosition)));
  }

  private Optional<Token> getEofToken() {
    if (startPosition == EOF) return Optional.of(new Token(TokenType.EOF, ""));
  ...
  private Optional<Token> getIntOrDoubleToken() {
    if (Character.isDigit(code.charAt(currentPosition))) {
      ...
      if (!(code.charAt(currentPosition) == '.')) {
        ...
        return Optional.of(
            new Token(TokenType.LIT_DOUBLE, code.substring(startPosition, currentPosition)));
      }
    }
    return Optional.empty();
  }
  ...
  private Optional<Token> getStringToken() {
    if (code.charAt(currentPosition) == '\"') {
      ...
      currentPosition++;
      return Optional.of(
          new Token(TokenType.LIT_STRING, code.substring(startPosition + 1, currentPosition - 1)));
    }
    ...
    if (Character.isLetter(code.charAt(currentPosition))) {
      do {
        currentPosition++;
          ...
          && (Character.isDigit(code.charAt(currentPosition))
              || Character.isLetter(code.charAt(currentPosition))));
  ...
  private Optional<Token> getOperatorToken() {
    if (tokenMap.isOperatorCharacter(code.charAt(currentPosition))) {
      ...
      if (tokenType != null) {
        ...
        if (tokenType != null) return Optional.of(new Token(tokenType, tokenText));
      }
    }
    return Optional.empty();
  }
